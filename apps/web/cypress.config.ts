...
// import plugin from './cypress/plugins/nock'
// const plugin = require('./cypress/plugins/nock');
  ...
  e2e: {
    async setupNodeEvents(on, config) {
      ...
      on('task', {
        ...
        async nock({ hostname, method, path, statusCode, body }: { hostname: string; method: string; path: string; statusCode: number; body: object; }) {
          nock.activate()
          ...
          nock(hostname)[method as 'get' | 'post'](path).reply(statusCode, body)
